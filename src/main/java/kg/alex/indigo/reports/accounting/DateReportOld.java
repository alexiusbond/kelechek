...
import kg.alex.indigo.pdf.AccountingByDatesPdf;
...
public class DateReportOld implements Button.ClickListener,
        Property.ValueChangeListener {

    static final Logger logger = LogManager.getLogger(DateReportOld.class);
    ...
    public DateReportOld(final MyVaadinUI ui, final HorizontalSplitPanel splitPanel) {
        this.myUI = ui;
            ...
            if (fromDateDF.isValid() && tillDateDF.isValid()) {
                if (!((Set<?>) incomeCategoriesTable.getValue()).isEmpty()
                    ...
                    try {
                        ...
                        expenseTtlLab.setValue("<b>" + myUI.getMessage(IndigoMessages.ExpensesTotal) + ": " + Settings.dFormat2.format(
                        ...
                        prev_balanceLab.setValue("<b>" + myUI.getMessage(IndigoMessages.PreviousBalance) + " (" + Settings.df.format(c.getTime()) + "): " + Settings.dFormat2.format(
                        ...
                        if (rightLayout.getComponentCount() != 0) {
                            ...
                            for (int i = 0; i < table_count && rightLayout.getComponent(i) instanceof Table; i++) {
                                ...
                                if (t.getContainerDataSource().size() != 0) {
                                    ...
                                    cell.setCellValue(myUI.getMessage(IndigoMessages.Accountant));
                                    cell.setCellStyle(style);
                                    cell = row.createCell(5);
                                    cell.setCellValue(myUI.getMessage(IndigoMessages.Director));
                                    cell.setCellStyle(style);
                                    row = excelReport.getWorkbook().getSheet(sheet).createRow(++rowNum);
                                    ...
                                    excelReport.getTotalsRow().getCell(excelReport.getTotalsRow().getFirstCellNum()).setCellValue(
                                            myUI.getMessage(IndigoMessages.IncomesTotal) + ": " + Settings.dFormat2.format(schoolAcc.getTotal_income()) + "$\t "
                                                    + myUI.getMessage(IndigoMessages.ExpensesTotal) + ": " + Settings.dFormat2.format(
                                                    schoolAcc.getTotal_outcome()) + "$\t "
                                                    + myUI.getMessage(IndigoMessages.PreviousBalance) + " (" + Settings.df.format(c.getTime())
                                                    + "): " + Settings.dFormat2.format(schoolAcc.getPrevious_balance()) + "$\t "
                                                    + myUI.getMessage(IndigoMessages.CashBox) + ": " + Settings.dFormat2.format(
