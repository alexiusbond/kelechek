...
import kg.alex.indigo.ui.SendOrderView;
...
public class DbOrderMessage extends BaseDb {
    ...
    public IndexedContainer execSQL(MyVaadinUI myUi, int employee_id, FilterTable t, SendOrderView view) throws SQLException {
        ...
        container.addContainerProperty(myUi.getMessage(IndigoMessages.OrderNumber), String.class, null);
        container.addContainerProperty(myUi.getMessage(IndigoMessages.Title), String.class, null);
        container.addContainerProperty(myUi.getMessage(IndigoMessages.Message), String.class, null);
        container.addContainerProperty(myUi.getMessage(IndigoMessages.Student), String.class, null);
        container.addContainerProperty(myUi.getMessage(IndigoMessages.Year), String.class, null);
        container.addContainerProperty(myUi.getMessage(IndigoMessages.Discount), Integer.class, null);
        ...
        while (result.next()) {
            ...
            if (result.getInt("om.student_id") == 0) {
                item.getItemProperty(myUi.getMessage(IndigoMessages.Student)).setValue(result.getString("student_info"));
            } else {
                item.getItemProperty(myUi.getMessage(IndigoMessages.Student)).setValue(result.getString("student"));
            }
            item.getItemProperty(myUi.getMessage(IndigoMessages.Year)).setValue(result.getString("y.name"));
            item.getItemProperty(myUi.getMessage(IndigoMessages.Discount)).setValue(result.getInt("discount"));
            item.getItemProperty(myUi.getMessage(IndigoMessages.Message)).setValue(result.getString("om.message"));
            item.getItemProperty(myUi.getMessage(IndigoMessages.Title)).setValue(result.getString("om.order_title"));
            item.getItemProperty(myUi.getMessage(IndigoMessages.OrderNumber)).setValue(result.getString("om.order_number"));
            item.getItemProperty(myUi.getMessage(IndigoMessages.Status)).setValue(result.getString("mst.name"));
            ...
            hl.setSpacing(true);
            hl.addComponent(view.createButton(myUi.getMessage(IndigoMessages.DeleteButton),
            ...
            hl.addComponent(view.createButton(myUi.getMessage(IndigoMessages.ViewDocument),
        ...
        if (t != null) {
            ...
            t.setColumnAlignment(myUi.getMessage(IndigoMessages.Discount), CustomTable.Align.RIGHT);
            t.setColumnFooter(myUi.getMessage(IndigoMessages.Title),
                    myUi.getMessage(IndigoMessages.UnRead) + ": " + unread);
            t.setColumnFooter(myUi.getMessage(IndigoMessages.Status),
                    myUi.getMessage(IndigoMessages.Total) + ": " + container.size());
        }
        return container;
    }
