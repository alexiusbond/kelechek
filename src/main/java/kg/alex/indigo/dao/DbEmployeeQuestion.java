...
import kg.alex.indigo.domain.EmployeeQuestioning;
...
public class DbEmployeeQuestion extends BaseDb {
    ...
    public IndexedContainer execSQL(MyVaadinUI myUI, int employee_id,
        ...
        container.addContainerProperty(myUI.getMessage(IndigoMessages.Question), String.class, null);
        container.addContainerProperty(myUI.getMessage(IndigoMessages.Answer), TextField.class, null);
        container.addContainerProperty(Settings.id, Integer.class, 0);
        while (result.next()) {
            String id = result.getString("q.id");
            Item item = container.addItem(id);
            item.getItemProperty(myUI.getMessage(IndigoMessages.Question)).setValue(result.getString("q.name"));
            item.getItemProperty(Settings.id).setValue(result.getInt("eq.id"));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Answer)).setValue(
                    edv.createTextField(result.getString("eq.answer"),
                            myUI.getMessage(IndigoMessages.Answer),
                            new StringLengthValidator(myUI.getMessage(IndigoMessages.NotificationWrongValue), null, 350, true), false));
        }
        return container;
    }
}
