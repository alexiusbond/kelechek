...
public class DbEmployeeLessons extends BaseDb {
    ...
    public IndexedContainer execSQL(MyVaadinUI myUI, int employee_id, int school_id,
        ...
        while (result.next()) {
            ...
            item.getItemProperty(Settings.button).setValue(
                    edv.createButton(myUI.getMessage(IndigoMessages.DeleteButton), id, Settings.dbEmployeeBranchHours, FontAwesome.MINUS_SQUARE));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Lesson)).setValue(
                    edv.createCombobox(result.getInt("ex.hr_branch_id"),
                            myUI.getMessage(IndigoMessages.Lesson), Settings.dbBranchTable, true));
            item.getItemProperty(myUI.getMessage(IndigoMessages.ClassName)).setValue(
                    edv.createCombobox(result.getInt("ex.class_number_id"),
                            myUI.getMessage(IndigoMessages.ClassName), Settings.classTable, true));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Lesson)).setValue(
                    edv.createCombobox(result.getInt("ex.hr_branch_id"),
                            myUI.getMessage(IndigoMessages.Lesson), Settings.dbBranchTable, true));
            item.getItemProperty(myUI.getMessage(IndigoMessages.AcademicYear)).setValue(
                    edv.createCombobox(result.getInt("ex.year_id"),
                            myUI.getMessage(IndigoMessages.AcademicYear), Settings.dbYear, true));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Hours)).setValue(
                    edv.createTextFieldWithProperty(result.getInt("ex.hours"), myUI.getMessage(IndigoMessages.Hours),
            ...
            item.getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).setValue(
                    edv.createTextFieldWithProperty(result.getInt("ex.extra_hours"), myUI.getMessage(IndigoMessages.ExtraHours),
    ...
    public IndexedContainer execSQL(MyVaadinUI myUI, int year_id, int school_id) throws SQLException {
        ...
        container.addContainerProperty(myUI.getMessage(IndigoMessages.Lecturer), String.class, null);
        container.addContainerProperty(myUI.getMessage(IndigoMessages.Lesson), String.class, null);
        container.addContainerProperty(myUI.getMessage(IndigoMessages.ClassName), Integer.class, 0);
        ...
        while (result.next()) {
            ...
            Item item = container.addItem(id);
            item.getItemProperty(myUI.getMessage(IndigoMessages.Lecturer)).setValue(result.getString("empl"));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Lesson)).setValue(result.getString("branch"));
            item.getItemProperty(myUI.getMessage(IndigoMessages.ClassName)).setValue(result.getInt("cn.name"));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Hours)).setValue(result.getInt("bh.hours"));
            item.getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).setValue(result.getInt("bh.extra_hours"));
        }
        return container;
    }
    ...
    public IndexedContainer execSQLHours(MyVaadinUI myUI, int employee_id, int school_id, int cl_num_id,
        ...
        container.addContainerProperty(myUI.getMessage(IndigoMessages.Lesson), String.class, null);
        container.addContainerProperty(myUI.getMessage(IndigoMessages.Hours), TextField.class, null);
        container.addContainerProperty(myUI.getMessage(IndigoMessages.ExtraHours), TextField.class, null);
        ...
        while (result.next()) {
            ...
            item.getItemProperty(myUI.getMessage(IndigoMessages.Lesson)).setValue(result.getString("br.name"));
            Integer hours = null;
            if (result.getInt("ebr.id") != 0) {
                hours = result.getInt("ebr.hours");
            }
            TextField tf = createTextfieldWithProperty(myUI, hours, myUI.getMessage(IndigoMessages.Hours),
                    new IntegerRangeValidator(myUI.getMessage(IndigoMessages.NotificationWrongValue), 1, 999),
                    new ObjectProperty<Integer>(0), Settings.getStringToIntegerConverter());
            tf.setData(result.getInt("ebr.id"));
            item.getItemProperty(myUI.getMessage(IndigoMessages.Hours)).setValue(tf);
            Integer extra_hours = null;
            if (result.getInt("ebr.id") != 0) {
                extra_hours = result.getInt("ebr.extra_hours");
            }
            item.getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).setValue(
                    createTextfieldWithProperty(myUI, extra_hours, myUI.getMessage(IndigoMessages.ExtraHours),
                            new IntegerRangeValidator(myUI.getMessage(IndigoMessages.NotificationWrongValue), 0, 999),
                            new ObjectProperty<Integer>(0), Settings.getStringToIntegerConverter()));
        }
        return container;
    ...
    public IndexedContainer execSQLHours(MyVaadinUI myUI, int year_id, int school_id,
        ...
        container.addItem("t");
        container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.ExtraBranches)).setValue(
                myUI.getMessage(IndigoMessages.TotalEmployeesAsExtraBranch) + ": 0");
        container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.MainBranch)).setValue(
                myUI.getMessage(IndigoMessages.TotalEmployeesAsMainBranch) + ": 0");
        while (result.next()) {
            if (branch_id != result.getInt("br.id")) {
                Item item = container.addItem("s" + result.getInt("br.id"));
                item.getItemProperty(myUI.getMessage(IndigoMessages.Lesson)).setValue(result.getString("br.name"));
                item.getItemProperty(myUI.getMessage(IndigoMessages.Employee)).setValue(myUI.getMessage(IndigoMessages.TotalEmployees) + "0");
                item.getItemProperty(myUI.getMessage(IndigoMessages.MainBranch)).setValue(myUI.getMessage(IndigoMessages.TotalEmployeesAsMainBranch)
                        + result.getString("br.name") + ": 0");
                item.getItemProperty(myUI.getMessage(IndigoMessages.ExtraBranches)).setValue(myUI.getMessage(IndigoMessages.TotalEmployeesAsExtraBranch)
                        + result.getString("br.name") + ": 0");
                totalEmpl = 0;
                totalAsMainBranch = 0;
                totalAsExtraBranch = 0;
                container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.Lesson)).setValue(
                        myUI.getMessage(IndigoMessages.TotalBranches) + (++totalLessons));
            }
            ...
            if (result.getInt("empl_t.br_id") != 0) {
                ...
                container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.Employee)).setValue(
                        myUI.getMessage(IndigoMessages.TotalEmployees) + (++totalEmpl));

                container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.Hours)).setValue(
                        ((Integer) container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.Hours)).getValue())
                                + result.getInt("empl_t.hours"));
                container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).setValue(
                        ((Integer) container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).getValue())
                                + result.getInt("empl_t.extra_hours"));
                container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.Hours)).setValue(
                        ((Integer) container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.Hours)).getValue())
                                + result.getInt("empl_t.hours"));
                container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).setValue(
                        ((Integer) container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.ExtraHours)).getValue())
                                + result.getInt("empl_t.extra_hours"));
                if (result.getInt("br.id") == result.getInt("empl_t.main_branch_id")) {
                    container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.MainBranch)).setValue(
                            myUI.getMessage(IndigoMessages.TotalEmployeesAsMainBranch) + result.getString("br.name") + ": " + (++totalAsMainBranch));
                    container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.MainBranch)).setValue(
                            myUI.getMessage(IndigoMessages.TotalEmployeesAsMainBranch) + ": " + (++grandTotalAsMainBranch));
                }
                if (result.getString("empl_t.extra_branch_ids") != null
                    ...
                    container.getItem("s" + result.getInt("br.id")).getItemProperty(myUI.getMessage(IndigoMessages.ExtraBranches)).setValue(
                            myUI.getMessage(IndigoMessages.TotalEmployeesAsExtraBranch) + result.getString("br.name") + ": " + (++totalAsExtraBranch));
                    container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.ExtraBranches)).setValue(
                            myUI.getMessage(IndigoMessages.TotalEmployeesAsExtraBranch) + ": " + (++grandTotalAsExtraBranch));
                }
            }

            container.getItem("t").getItemProperty(myUI.getMessage(IndigoMessages.Employee)).setValue(
                    myUI.getMessage(IndigoMessages.TotalEmployees) + empCount.size());
        }
        return container;
    }
