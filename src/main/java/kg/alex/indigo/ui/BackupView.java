...
public class BackupView extends HorizontalSplitPanel implements Button.ClickListener {
    ...
    public BackupView(MyVaadinUI myUI) {
        ...
        takeBakup = new Button(myUI.getMessage(IndigoMessages.TakeBackupButton));
    ...
    public void buttonClick(Button.ClickEvent event) {
        ...
        if (source == takeBakup) {
            try {
                ProcessBuilder pb = new ProcessBuilder("/home/indigo/backup.sh", "myArg1", "myArg2");
                pb.start();
            } catch (IOException e) {
                logger.error(e);
                logger.catching(e);
            ...
            try {
                myUI.getPage().open(new FileResource(
                                new File("/home/indigo/backups/" + source.getData())),
                        "_blank", false);
            } catch (Exception e) {
                logger.error(e);
                logger.catching(e);
    ...
    private Button createButton(String itemId) {
        Button btn = new Button(myUI.getMessage(IndigoMessages.DownloadBackupButton));
    ...
    private void setDataTable() {
        ...
        try {
            File folder = new File("/home/indigo/backups");
            ...
            dataTable.clear();
            container.sort(new String[]{myUI.getMessage(IndigoMessages.Date)},
                    new boolean[]{false});
