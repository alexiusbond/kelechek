    ...
    public ContractSchoolPdf_2025_ru(final MyVaadinUI ui, StudentInfoPdf st_info, final IndexedContainer instPlanCont) {
        ...
        StreamResource.StreamSource source1 = () -> {
            ...
            try {
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.3.13. Родители признают основные ценности школы такие как благодарность, уважение, дружелюбие и ответственность. Обязуется поддерживать их соблюдение в школе и сотрудничать с учителями. А также обязуется поддерживать обучающие мероприятия и программы, направленные на развитие этих ценностей. Родители соглашаются сотрудничать с учителями и администрацией школы для обеспечения эффективной реализации ценностно ориентированного обучения. И готовы участвовать в родительских собраниях, консультациях и других мероприятиях, направленных на поддержку обучения наших детей.", ordFont));
                document.add(paragraph);
