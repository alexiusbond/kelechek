...
public class ContractAsylkechPdf_2025_ru {

    static final Logger logger = LogManager.getLogger(ContractAsylkechPdf_2025_ru.class);
    ...
    public ContractAsylkechPdf_2025_ru(final MyVaadinUI ui, StudentInfoPdf st_info, final IndexedContainer instPlanCont) {
        this.myUI = ui;
        ...
        StreamResource.StreamSource source1 = () -> {
            ...
            try {
                ...
                paragraph.clear();
                paragraph.add(new Phrase("1.2. tРодители обязуются вносить оплату за образовательные услуги согласно пункту 3 (Условия оплаты Родителей за образовательные услуги) данного Договора.", ordFont));
                document.add(paragraph);
                document.add(new Paragraph(10, " "));
                ...
                paragraph.add(new Phrase("Родителем взноса, с извещением Родителя ограничить доступ посещения Обучающегося ко всем формам учебных и не учебных занятий и использования материально-технических условий (занятий, библиотеки, столовой, этюдов и тд., не допускать на экзамены, не выставлять оценки в электронном журнале “Эдупэйдж”, ). Помимо этого, без предупреждения Родителя ", ordFont));
                paragraph.add(new Phrase("аннулируются ", ordBoldFont));
                paragraph.add(new Phrase("предоставленные Школой скидки.", ordBoldFont));
                document.add(paragraph);
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.2.6.	Удержать сумму родительского взноса за текущую четверть при расторжении настоящего договора по инициативе Родителя, при этом ранее предусмотренные скидки не учитываются.", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.7.	При расторжении настоящего договора по непредвиденным обстоятельствам, с учетом всех понесенных расходов школы, сумма подлежащая к возврату, возвращается по мере возможности Школы, но не позднее мая следующего года.", ordFont));
                document.add(paragraph);
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.3.11.	Своевременно предоставлять требуемые школой документы.", ordFont));
                document.add(paragraph);
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.3.13.	Родители признают основные ценности школы, прописанные в названии «Асылкеч». Обязуются поддерживать их соблюдение в школе и сотрудничать с учителями. А также обязуются поддерживать обучающие мероприятия и программы, направленные на развитие этих ценностей. Родители соглашаются сотрудничать с учителями и администрацией школы для обеспечения эффективной реализации ценностно-ориентированного обучения. И готовы участвовать в родительских собраниях, консультациях и других мероприятиях, направленных на поддержку обучения наших детей.", ordFont));
                document.add(paragraph);
                ...
                paragraph.clear();
                paragraph.setAlignment(Element.ALIGN_JUSTIFIED);
                paragraph.add(new Phrase("3.1. Ежегодно с 1 апреля по 20 мая Родители обязаны заключить договор на образовательные услуги на следующий учебный год, ", ordFont));
                ...
                paragraph.add(new Phrase("31 го марта ", ordBoldFont));
                paragraph.add(new Phrase("следующего года. Размер родительского взноса не изменяется даже при условии перехода Школы на дистанционную форму обучения.", ordFont));
                document.add(paragraph);
