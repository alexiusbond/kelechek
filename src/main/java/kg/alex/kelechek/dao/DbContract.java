...
public class DbContract extends BaseDb {
    ...
    public IndexedContainer exec_contr_select(
            MyVaadinUI myUi, int year_id, int school_id, int contract_id)
            throws SQLException {
        String sql = "select t.id, t.name, t.amount, y.name " +
                     "from contract as t " +
