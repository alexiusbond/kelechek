...
import kg.alex.aim.reports.students.ClassPaymentsReport;
import kg.alex.aim.reports.students.InstallmentPlanPaymentsReport;
...
public class DbStudentPayment extends BaseDb {
    ...
    public String getOrderNum(String id) throws SQLException {
        ...
        ResultSet result = stat.executeQuery();
        if (result.next()) {
            orderNum = String.format("%07d", (result.getInt("order_number")));
        } else {
            orderNum = String.format("%07d", 1);
        }
        return orderNum;
    }
