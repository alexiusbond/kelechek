...
import kg.alex.aim.domain.Definition;
import kg.alex.aim.domain.EmployeeWork;
import kg.alex.aim.i18n.Messages;
import kg.alex.aim.ui.EmployeeDefinitionView;
...
public class DbEmployeeWork extends BaseDb {
    ...
    public IndexedContainer execSQL(final MyVaadinUI myUI, int employee_id, int own_id,
        ...
        while (result.next()) {
            ...
            item.getItemProperty(myUI.getMessage(Messages.Ellipse)).setValue(
                    edv.createCheckBox(result.getBoolean("ew.is_sapat"), myUI.getMessage(Messages.Ellipse)));
    ...
    public IndexedContainer execSQL(final MyVaadinUI myUI, int employee_id, int own_id) throws SQLException {
        ...
        container.addContainerProperty(myUI.getMessage(Messages.Ellipse), String.class, null);
        ...
        while (result.next()) {
            ...
            item.getItemProperty(myUI.getMessage(Messages.Ellipse)).setValue(result.getInt("ew.is_sapat") == 1 ?
